function [mssim, ssim_map, img1_extern, img1_squared_extern, mu11_extern, mu1_sq_extern, mu1111_extern] = ssim1_pyssim_padarray(img1, img2, K, window, L)
...
if (columns(size(img1))!=3 || columns(size(img2))!=3)
   mssim = 123;
   ssim_map = 123;
   return;
end
...
img1=floor(114/1000*double(img1(:,:,3))+587/1000*double(img1(:,:,2))+299/1000*double(img1(:,:,1))+0.00000001);
img2=floor(114/1000*double(img2(:,:,3))+587/1000*double(img2(:,:,2))+299/1000*double(img2(:,:,1))+0.00000001);
img1_extern=img1;

img1_squared = img1 .* img1;
img2_squared = img2 .* img2;
img1_squared_extern=img1_squared;

C1 = (K(1)*L)^2;
C2 = (K(2)*L)^2;
% window = window/sum(sum(window));

mu1   = filter2(window, padarray(img1', [0 5], "symmetric"))(:,6:1085)';
mu11  = filter2(window, padarray(mu1, [0 5], "symmetric"))(:,6:1925);
mu11_extern=mu11;
mu1_sq = mu11.*mu11;
mu1_sq_extern=mu1_sq;
mu111 = filter2(window, padarray(img1_squared', [0 5], "symmetric"))(:,6:1085)';
mu1111= filter2(window, padarray(mu111, [0 5], "symmetric"))(:,6:1925);
mu1111_extern=mu1111;
